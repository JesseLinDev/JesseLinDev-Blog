...
the site was loading fonts from google's cdn. worked fine, but for better international support and consistency, i switched to a multi-font system. google sans for latin text, noto sans sc for chinese characters, and google sans mono for code. self-hosted, multiple font weights, woff2 format. no external font requests. the font loads from the same place as everything else.
