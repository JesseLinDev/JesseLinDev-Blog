...
// 在Node.js环境中直接读取WASM文件
async function initWasmNode(): Promise<void> {
  if (wasmInitialized) return;
  
  try {
    const currentDir = dirname(fileURLToPath(import.meta.url));
    const wasmPath = join(currentDir, 'resvg.wasm');
    const wasmBuffer = readFileSync(wasmPath);
    await initWasm(wasmBuffer);
    wasmInitialized = true;
    console.log('WASM initialized successfully from filesystem');
  } catch (e) {
    console.error('Failed to initialize WASM from filesystem:', e);
